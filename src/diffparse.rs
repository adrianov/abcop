//! Unified-diff parsing for change scopes: turns a `git diff -U0 -W`
//! output into per-file new-side line sets.
...
fn hunk_spec(rest: &str) -> Option<(usize, usize)> {
    let mut parts = spec_digits(rest)?.split(',');
    let start = parts.next()?.parse::<usize>().ok()?;
    let count = parts.next().and_then(|v| v.parse().ok()).unwrap_or(1);
    Some((start, count))
}

/// The leading `+<digits>[,<digits>]` run of a hunk header tail.
fn spec_digits(rest: &str) -> Option<&str> {
    let plus_idx = rest.find('+')?;
    let tail = &rest[plus_idx + 1..];
    let digits_end = tail
        .find(|c: char| !(c.is_ascii_digit() || c == ','))
        .unwrap_or(tail.len());
    Some(&tail[..digits_end])
}
